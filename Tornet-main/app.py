...
def login():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password'] # admin123
...
def dashboard():
    ...
    if tasks != None:
        return render_template('dashboard.html',tasks=tasks, api_key = api_key, secret_access_key = secret_access_key)
    else:
        return render_template('dashboard.html', api_key = api_key, secret_access_key = secret_access_key)
...
#@login_required # Commented the login required because it should create error in scheduling tasks.
def discover():
    threads = []
...
# 
    ...
    #            urls.add(link)     # Adding element in a set.
                #urls.append(f"http://{link}")
                f.write(f"http://{link}\n")
                try:
                    # Use requests library instead of subprocess for better performance
                    if cnt==2:
                        # print(cnt)
                        break 
                    # print(cnt)
                    link = f"http://{link}"
                    print(f"\nLINK : {link}\n")
                    response = requests.head(link, timeout=10, proxies=proxy)
                    print(f"\nRESPONSE : {response}\n")
                    cnt+=1
                    #link = f"http://{link}"
    ...
    # Append results to the list
                urls.append({"url": link, "status": status})
...
#    print(api_key,secret_access_key) # If you want to show the parsed keys
    ...
    if task_route == '/enumerate/discover':
        ...
        if response.status_code:
            regexquery = r"\w+\.onion"
            onion_links = re.findall(regexquery, response.text)
...
def send_email_notification(email, task_route, task_result):
    ...
    try:
        ...
        sender_email = 'prakharporwal2004@gmail.com'
        password = 'bfqn sgdx wbly zfgk'
...
def upload_file():
    if request.method == "POST":
        ...
        if uploaded_file.filename != "" and allowed_file(uploaded_file.filename):
            # Save file
            uploaded_file.save(f"internal/uploads/{uploaded_file.filename}")
            ...
            with open(f"internal/uploads/{uploaded_file.filename}", "r") as f:
                for line in f:
                    url = line.strip()

                    # Start thread and pass url as argument
                    thread = threading.Thread(target=worker, args=(all_urls, url))
                    thread.start()
                    threads.append(thread)
            ...
            active_urls = sum(url["status"] == "Active" for url in all_urls)
            dead_urls = len(all_urls) - active_urls

            with open("internal/validate/export_links.txt", "w") as f:     
                all_urls.append(url)
                f.write(f"{url}\n")
            f.close()
            ...
            return render_template("validate.html", message="File processed successfully!",
                                    active_urls=active_urls, dead_urls=dead_urls,
                                    all_urls=all_urls)
        else:
...
def worker(all_urls, url):
    response = ""
    try:
        # Use requests library instead of subprocess for better performance
        url=f"http://{url}"
        response = requests.head(url, timeout=20, proxies=proxy)
    ...
    # Append results to the list
    all_urls.append({"url": url, "status": status, "response": response})
...
def render():
    if request.method == "POST":
        ...
        try:
            ...
            # Get DOM content
            dom_content = response.content.decode()
...
regex_patterns = {
    ...
    'BASEPATHS': r'(?<="|\')http?://'+'f{url}'+'[^\s"\']+',
    'regexquery' : r"\w+\.onion"  # Basepaths, Directories, Files
}
